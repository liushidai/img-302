package com.example.img302.common.redis;
...
public class RedisTemplateHelper {
     ...
     */
    private void scanShortMax(String pattern, Consumer<byte[]> consumer) {
        stringRedisTemplate.execute((RedisConnection connection) -> {
            try (Cursor<byte[]> cursor = connection.scan(ScanOptions.scanOptions().count(Short.MAX_VALUE).match(pattern).build())) {
     ...
     * @return
     */
    public Set<String> scanShortMax(String pattern) {
        Set<String> keys = new HashSet<>();
        this.scanShortMax(pattern, item -> {
            // 符合条件的key
     ...
     * @param consumer 对迭代到的key进行操作
     */
    private void scanByteMax(String pattern, Consumer<byte[]> consumer) {
        stringRedisTemplate.execute((RedisConnection connection) -> {
            try (Cursor<byte[]> cursor = connection.scan(ScanOptions.scanOptions().count(Byte.MAX_VALUE).match(pattern).build())) {
                cursor.forEachRemaining(consumer);
                return null;
            }
        });
     ...
     * @return
     */
    public Set<String> scanByteMax(String pattern) {

        Set<String> keys = new HashSet<>();
        this.scanByteMax(pattern, item -> {
            // 符合条件的key
            String key = new String(item, StandardCharsets.UTF_8);
            keys.add(key);
        });
    ...
    public void deleteByPattern(String pattern) {
        ...
        Set<String> keys = this.scanShortMax(pattern);
    ...
    public Boolean expireAt(String key, LocalDateTime localDateTime) {
        return stringRedisTemplate.expireAt(key, Date.from(localDateTime.atZone(ZoneId.systemDefault()).toInstant()));
    }
    ...
    public Long geoAdd(String key, Double x, Double y, String member) {
        return stringRedisTemplate.opsForGeo().add(key, new Point(x, y), member);
    }
     ...
     * @return
     */
    public Long geoDelete(String key, String member) {
     ...
     * @param x
     * @param y
     * @param distance 距离（半径）
     */
    public GeoResults<RedisGeoCommands.GeoLocation<String>> geoRadius(String key, Double x, Double y, Integer distance) {
